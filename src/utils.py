...
def generate_file_path(filename):
    return join(config.Paths_Files.SRC_PATH+config.Paths_Files.TMP_DIR, f"{datetime.now().strftime('%Y-%m-%d_%H:%M:%S.%f')}-{filename}")

def save_transcript_to_audio(transcript, transcript_filename = config.Paths_Files['FILE_NAME_TRANSCRIPT']):
...
def save_audio_to_file(audio_data: np.ndarray, file_audio_path) -> None:
    ...
    try:
        if audio_data is None or not isinstance(audio_data, np.ndarray):
            ...
            return
        sf.write(file=file_audio_path, data=audio_data, samplerate=config.Audio.SAMPLE_RATE)
    except IOError:
